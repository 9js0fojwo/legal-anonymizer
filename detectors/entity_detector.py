...
# 用户指定姓氏列表（90个）
COMMON_SURNAMES = set(
    '王李张刘陈杨赵黄周吴徐孙胡朱高林何郭马罗梁宋郑谢韩唐冯于董萧程曹袁邓许傅沈曾彭吕苏卢蒋蔡贾丁魏薛叶阎余潘杜戴夏钟汪田任姜范方石姚谭廖邹熊金陆郝孔白崔康毛邱秦江史顾侯邵孟龙万段章钱汤'
)
...
class EntityDetector:
    ...
    def detect(self, text: str, only_types: List[str] = None, exclude_types: List[str] = None) -> List[Tuple[str, str, int]]:
        ...
        person_names = [
            ...
            if etype == 'person' and 2 <= len(name) <= 3
        ]
        ...
        if self._should_detect('person', only_types, exclude_types):
            for keyword in PERSON_CONTEXT_KEYWORDS:
                ...
                pattern_with_sep = re.compile(
                    rf'{re.escape(keyword)}[：:、\s]+\s*([\u4e00-\u9fa5]{{2,3}})(?=[，。、；：\s\n的了是为系先女男诉向不等因被涉（(教博士硕]|$)'
                )
                ...
                pattern_verb_sep = re.compile(
                    rf'{re.escape(keyword)}(?:为|是|系)([\u4e00-\u9fa5]{{2,3}})(?=[，。、；：\s\n先女男教博士硕（(]|$)'
                )
            ...
            reverse_pattern = re.compile(
                rf'([\u4e00-\u9fa5]{{2,3}})\s*(?:亦|也|又|均|都)?\s*(?:是本案|是|系|为)\s*(?:公司|本公司|目标公司|本案)?\s*(?:{reverse_roles_str})'
            )
            ...
            multi_name_role = re.compile(
                r'([一-龥]{2,3})(?=[、，,和与及分作担任为是系出\s的])'
                r'\s*[、，,和与及]\s*'
                r'([一-龥]{2,3})(?=[、，,和与及分作担任为是系出\s的])'
                r'(?:\s*[、，,和与及]\s*([一-龥]{2,3})(?=[、，,和与及分作担任为是系出\s的]))?'
            ...
            id_patterns = [
                r'([\u4e00-\u9fa5]{2,3})\s*[\d\W]*\s*身份证号',
                r'([\u4e00-\u9fa5]{2,3})\s*开[户己]行',
                r'([\u4e00-\u9fa5]{2,3})\s*[（(]\s*(?:签字|签章|盖章)\s*[）)]',
                r'(?:甲方|乙方|丙方|丁方)[：:]\s*([\u4e00-\u9fa5]{2,3})(?=[，。\s\d])',
                # "张三向XX法院提出" - NAME + 向 + 机构
                r'([\u4e00-\u9fa5]{2,3})\s*向\s*[\u4e00-\u9fa5]{2,}(?:法院|检察院|仲裁委)',
                # "判处/判决 张三 死刑/有期徒刑" - 刑事判决中的被告人
                r'判[处决]\s*([\u4e00-\u9fa5]{2,3})\s*(?=(?:死刑|有期徒刑|无期徒刑|拘役|管制|罚金|缓刑))',
                # "张三系XX有限公司法定代表人" - 系/是 + 公司名 + 法定代表人（允许较长中间文本）
                r'([\u4e00-\u9fa5]{2,3})\s*系[\u4e00-\u9fa5（）\s]{3,60}?法定代表人',
            ]
